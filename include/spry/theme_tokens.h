...
// The core theme-token vocabulary (#321). These are the color and metric names Spry's built-in
// widgets read from the active Theme. Widgets reference these constants instead of ad-hoc string
// literals, so the vocabulary has one authoritative, documented home. A Theme should define
// them; when a token is absent a widget uses its own fallback (missing tokens degrade quietly
// rather than crash). Hosts may define any number of extra custom tokens beyond these.
namespace spry {
namespace tokens {
...
// The core tokens as arrays, for iteration and validation (see Theme::missingCoreTokens()).
